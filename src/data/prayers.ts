export interface Prayer {
  ...
  title: string;
  category: 'daily' | 'marian' | 'devotion' | 'litany' | 'rosary' | 'chaplet' | 'novena';
  text: string;
}
...
export function prayersByCategory() {
  const categories = {
    ...
    chaplet:  BUILTIN_PRAYERS.filter(p => p.category === 'chaplet'),
    novena:   BUILTIN_PRAYERS.filter(p => p.category === 'novena'),
  };
  return categories;
}
