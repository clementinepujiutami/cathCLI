...
exports.halftonePanel = halftonePanel;
exports.prayerCard = prayerCard;
...
function halftonePanel(art, w, h = 14) {
    const srcLum = (0, render_1.toLuminance)(art);
    ...
    if (fitW > w) {
        fitW = w;
        fitH = Math.max(1, Math.round(((srcH / srcW) * fitW) / 2));
    }
    ...
    for (let y = 0; y < h; y++) {
        const row = [];
        for (let x = 0; x < w; x++)
            row.push(scaled[y - y0]?.[x - x0] ?? 0);
        lum.push(row);
    }
    return (0, render_1.renderHalftone)(lum, {
        bg: t => (0, render_1.mix)(CARD.panelFrom, CARD.panelTo, t),
        fg: t => (0, render_1.mix)(CARD.inkLo, CARD.inkHi, t),
    });
}
function prayerCard(title, text, opts = {}) {
    ...
    const flat = chalk_1.default.level <= 1;
    ...
    const line = (content = '') => {
        if (flat)
            return (' '.repeat(pad) + content).replace(/\s+$/, '');
        const gap = Math.max(0, inner - visLen(content));
        return chalk_1.default.bgHex(CARD.bg)(' '.repeat(pad) + content + ' '.repeat(gap + pad));
    };
    const out = [];
    out.push('');
    if (opts.art && !flat) {
        for (const row of halftonePanel(opts.art, w))
            out.push(row);
    }
    out.push(line());
    ...
    for (const l of wordWrap(title.toUpperCase(), inner)) {
        out.push(line(chalk_1.default.hex(CARD.head).bold(l)));
    }
    if (opts.category) {
        for (const l of wordWrap(opts.category.toUpperCase(), inner)) {
            out.push(line(chalk_1.default.hex(CARD.sub)(l)));
        }
    }
    out.push(line());
    for (const l of wordWrap(text, inner))
        out.push(line(chalk_1.default.hex(CARD.body)(l)));
    out.push(line());
    if (opts.tags?.length) {
        ...
        const cells = opts.tags.map(t => {
            ...
            const s = chalk_1.default.hex(CARD.sub)('> ') +
                (flat ? label : chalk_1.default.bgHex(CARD.tagBg).hex(CARD.tagFg)(` ${label} `));
            return { s, w: visLen(s) };
        });
        let cur = [];
        let curW = 0;
